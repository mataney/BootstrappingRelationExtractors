...
experiments = ['byiddwba8171-2']

relations = ["country_of_origin"]
...
def main():
    with open(ALL_OUTPUTS_FILE, 'w') as all_outputs_file:
        ...
        for experiment, relation in zip(experiments, relations):
            cmd = f"gsutil cp gs://oto-experiments/{experiment}/full_results.json temp_results_file.json"
            os.system(cmd)
            ...
            confidence_threshold_on_dev_eval = 0.5

...
def prepare_predictions(args, predictions):
    ret = [{'guid': p['guid'], 'c': float(p['c'])} for p in predictions if p['r'] == args.relation_name]
    ret = sorted(ret, key=lambda x: x['c'], reverse=True)
    return ret

def score(key, predictions, args):
    best_f1, best_confidence = 0, (0.5 - 1e-10)
    relevant_relation_predictions = prepare_predictions(args, predictions)
    key_by_guid = {k['id']: k['relation'] for k in key}

    gold_in_label = sum([1 for k in key if k['relation'] == args.relation_name])
    pred_in_label = len(relevant_relation_predictions)
    ...
    for i, pred in enumerate(relevant_relation_predictions):
        id = pred['guid']
        gold_label = key_by_guid[id]

        if pred['c'] < args.confidence_threshold:
            break
         
        if gold_label == args.relation_name:
...
if __name__ == "__main__":
    ...
    predictions = read_json(args.pred_file)

    # Score the predictions
    score(key, predictions, args)
